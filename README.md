...
タイマー中にアプリから離れた瞬間にプッシュ通知が届きます。あらかじめ登録した「目的」「なぜ」と、選んだ AI の性格に合わせて、その時のあなた専用に AI が生成したメッセージで自然と集中へ引き戻します。
...
- **🤖 AI のひと言で集中へ引き戻す**: アプリから離れた瞬間にプッシュ通知が届き、テンプレではない“あなた専用のメッセージ”で集中へ戻れる。
- **🎭 6 種類から選べる AI の性格**: `サポーター` / `ライバル` / `クール` / `コーチ` / `剣士` / `いたずらっ子` から、その日の気分に合わせて切り替え。
- **📅 ヒートマップで継続を可視化**: 日々の集中時間をカレンダーで色濃く積み上げ、続ける自信に変える。
- **🎯 目的と「なぜ」を AI が活用**: 志望校合格・資格取得・副業など、あなたの想いを踏まえた言葉で背中を押します。
